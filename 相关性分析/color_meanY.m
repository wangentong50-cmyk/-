...
% 行：筘数-纬数，列：颜色
meanY_matrix = NaN(length(uniqueCombs), length(colors));
...
for ci = 1:length(colors)
    ...
    for i = 1:length(uniqueCombs)
        ...
        if any(idx)
            meanY_matrix(i, ci) = mean(T.MeanY(idx));
        end
    end
end

% 绘图
hFig = figure('Name', 'MeanY-Color-Line', 'NumberTitle', 'off');
hold on;
